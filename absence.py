...
# Step 2: Initialize attendance and absence tracking
attendance_log = []
absence_log = []
absence_start = None  # Track when the absence period starts
...
while True:
    ...
    verified_face_detected = False  # Flag to track if any verified face is found
    ...
    if not verified_face_detected:
        if absence_start is None:
            absence_start = datetime.now()
    else:
        if absence_start is not None:
            absence_end = datetime.now()
            absence_duration = f"{absence_start.strftime('%Y-%m-%d %H:%M:%S')} to {absence_end.strftime('%Y-%m-%d %H:%M:%S')}"
            absence_log.append(absence_duration)
            print(f"Absent from {absence_duration}")
            absence_start = None  # Reset absence start
...
# Save attendance and absence logs
...
with open("absence_log.txt", "w") as log_file:
    log_file.write("Absence Log\n")
    log_file.write("===================\n")
    for log in absence_log:
        log_file.write(f"{log}\n")

print("Logs saved: attendance_log.txt and absence_log.txt.")
